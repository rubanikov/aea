...
Concretely:
- **DB layer:** Supabase RLS policies keyed on `auth.uid()` — `patient_id = auth.uid()` on `Booking` reads for the patient role, `provider_id = auth.uid()` for the provider role, admin bypasses via a separate policy scoped and logged.
- **App layer:** every service method that touches a `Booking`, `Availability`, or PHI field re-checks ownership explicitly, rather than trusting RLS alone. Redundant by design — the brief's "Postgres RLS **or** explicit server-side checks" becomes "both," matching what every system that takes this seriously does.
