 ...
 * from their stored `GET /profile` value. Deliberate choice for TICKET-06:
 * the ticket's accept criterion is that a slot displays unambiguously in
 * "the patient's local zone" -- where they physically are right now, which
 * is what actually determines what time it reads on their wall clock -- not
 * necessarily wherever they last set their profile to. A traveling
 * patient's stored profile timezone can silently go stale; the browser's
 * zone can't, since it's re-detected on every load. (A stored-profile
 * selector, matching `AppointmentTypesSection`'s read-only `GET /profile`
 * timezone line, is also defensible -- this is a judgment call, not a
 * uniquely correct answer; see this ticket's handoff notes.)
 *
 * Built on `useSyncExternalStore` rather than a `useState` + `useEffect`
 * pair -- this is exactly the case it exists for (React's own docs use the
 ...
 * pre-hydration pass -- a real, if brief, loading state for callers to
 * show something for, never silently defaulting to `"UTC"`, which could
