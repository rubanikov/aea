...
too, so this is the *only* place `Booking.status` is ever assigned outside a
migration -- no ad-hoc status writes anywhere else.
"""
...
# No cancel within 24h of start. architecture.md doesn't mandate a
# different value, so this is the one hardcoded minimum-notice window the
# whole app enforces -- see `CancellationNoticeTooShort`.
...
def check_cancellation_notice(booking):
    ...
    this comparison is written, so it never becomes two
    independently-maintained checks. `transition()` calls this for every
    `-> CANCELLED` transition (see below); `bookings.services
    .reschedule_booking` calls it too, directly, against the *original*
    booking, before it even attempts the new slot.
    """
...
def transition(booking, new_status, *, actor, enforce_notice=True, cancellation_reason=None):
    ...
    - `NoShowBeforeStartTime` if `new_status` is `NO_SHOW` and
      ...
      in UTC -- checked only *after* the table lookup above confirms
      `confirmed -> no_show` is otherwise legal, so a booking that isn't
      even `confirmed` gets the generic `InvalidTransition` instead.
    - `CancellationNoticeTooShort` if `new_status` is `CANCELLED` and
      `booking.start_time` is less than `CANCELLATION_MIN_NOTICE` away
      from real "now" -- same ordering as the `NO_SHOW` rule above, and
      applied regardless of actor: this doesn't distinguish a patient's
      own cancellation from a provider's or an admin's.

    `actor` is required and explicit at every call site (never defaulted)
    -- pass `None` for a system-initiated transition (e.g. the
    ...
    that passes `False` is
    `accounts.services._cancel_upcoming_appointments` (the
    account-deletion flow): there is no "too late to cancel" left to
    protect once the account itself is going away, so that single call
    site deliberately skips this check rather than blocking deletion (or
    ...
    `cancellation_reason`: the provider's written reason, already
    validated by `BookingStatusUpdateSerializer` (non-blank, <= 500
    chars, only ever present for a `-> CANCELLED` transition). Written to
    `Booking.cancellation_reason` in the *same* `save()` as the status
    change, so the two can never disagree; a rejected transition leaves
    the stored reason untouched. The default `None` means "leave the
    field alone," which is what every cancel path other than the
    provider status endpoint passes implicitly, keeping their bookings'
    reasons `""`.
    ...
    with transaction.atomic():
        ...
        record_audit_event(
            ...
            # text or contact details (architecture.md §6, "no PHI in
            # logs"). The reason lives on the Booking row instead.
